...
encode(#ssh_msg_global_request{name = Name,
                               want_reply = Bool,
                               data = Data}) ->
...
encode(#ssh_msg_channel_open{channel_type = Type,
                             sender_channel = Sender,
                             initial_window_size = Window,
                             maximum_packet_size = Max,
                             data = Data}) ->
    <<?Ebyte(?SSH_MSG_CHANNEL_OPEN),
      ?Estring(Type),
      ?Euint32(Sender),
      ?Euint32(Window),
      ?Euint32(Max),
      ?'E...'(Data)>>;
encode(#ssh_msg_channel_open_confirmation{recipient_channel = Recipient,
                                          sender_channel = Sender,
    ...
    <<?Ebyte(?SSH_MSG_CHANNEL_OPEN_CONFIRMATION),
      ?Euint32(Recipient),
      ?Euint32(Sender),
      ?Euint32(InitWindowSize),
      ?Euint32(MaxPacketSize),
      ?'E...'(Data)>>;
encode(#ssh_msg_channel_open_failure{recipient_channel = Recipient,
                                     reason = Reason,
                                     description = Desc,
                                     lang = Lang}) ->
    <<?Ebyte(?SSH_MSG_CHANNEL_OPEN_FAILURE),
      ?Euint32(Recipient),
      ?Euint32(Reason),
      ?Estring(Desc),
      ?Estring(Lang)>>;
encode(#ssh_msg_channel_window_adjust{recipient_channel = Recipient,
                                      bytes_to_add = Bytes}) ->
...
encode(#ssh_msg_channel_extended_data{recipient_channel = Recipient,
                                      data_type_code = DataType,
                                      data = Data}) ->
    <<?Ebyte(?SSH_MSG_CHANNEL_EXTENDED_DATA),
      ?Euint32(Recipient),
      ?Euint32(DataType),
      ?Ebinary(Data)>>;
...
encode(#ssh_msg_channel_request{recipient_channel = Recipient,
                                request_type = Type,
                                want_reply = Bool,
                                data = Data}) ->
    <<?Ebyte(?SSH_MSG_CHANNEL_REQUEST),
      ?Euint32(Recipient),
      ?Estring(Type),
      ?Eboolean(Bool),
      ?'E...'(Data)>>;
...
encode(#ssh_msg_userauth_request{user = User,
                                 service = Service,
                                 method = Method,
                                 data = Data}) ->
    <<?Ebyte(?SSH_MSG_USERAUTH_REQUEST),
      ?Estring_utf8(User),
      ?Estring(Service),
      ?Estring(Method),
      ?'E...'(Data)>>;
...
encode(#ssh_msg_userauth_passwd_changereq{prompt = Prompt, language = Lang}) ->
    <<?Ebyte(?SSH_MSG_USERAUTH_PASSWD_CHANGEREQ), ?Estring_utf8(Prompt), ?Estring(Lang)>>;
encode(#ssh_msg_userauth_info_request{name = Name,
                                      instruction = Inst,
                                      language_tag = Lang,
                                      num_prompts = NumPromtps,
                                      data = Data}) ->
    <<?Ebyte(?SSH_MSG_USERAUTH_INFO_REQUEST),
      ?Estring_utf8(Name),
      ?Estring_utf8(Inst),
      ?Estring(Lang),
      ?Euint32(NumPromtps),
...
encode(#ssh_msg_userauth_info_response{num_responses = Num, data = Data}) ->
    lists:foldl(fun%%("", Acc) -> Acc;  % commented out since it seem wrong
                ...
                Data);
encode(#ssh_msg_disconnect{code = Code,
                           description = Desc,
                           language = Lang}) ->
...
encode(#ssh_msg_kexdh_reply{public_host_key = {Key, SigAlg},
                            f = F,
                            h_sig = Signature}) ->
...
encode(#ssh_msg_kex_dh_gex_request{min = Min,
                                   n = N,
                                   max = Max}) ->
...
encode(#ssh_msg_kex_dh_gex_reply{%% Will be private key encode_host_key extracts only the public part!
...
encode(#ssh_msg_kex_ecdh_reply{public_host_key = {Key, SigAlg},
                               q_s = Q_s,
                               h_sig = Sign}) ->
    EncKey = ssh2_pubkey_encode(Key),
...
encode(#ssh_msg_kex_hybrid_reply{public_host_key = {Key, SigAlg},
                                 s_reply = S_reply,
                                 h_sig = Sign}) ->
    EncKey = ssh2_pubkey_encode(Key),
    EncSign = encode_signature(Key, SigAlg, Sign),
    <<?Ebyte(?SSH_MSG_KEX_HYBRID_REPLY),
      ?Ebinary(EncKey),
      ?Ebinary(S_reply),
      ?Ebinary(EncSign)>>;
...
%% Connection Messages
decode(<<?BYTE(?SSH_MSG_GLOBAL_REQUEST),
         ?DEC_BIN(Name, __0),
         ?BYTE(Bool),
         Data/binary>>) ->
...
decode(<<?BYTE(?SSH_MSG_CHANNEL_OPEN),
         ...
         ?UINT32(Max),
         Data/binary>>) ->
    #ssh_msg_channel_open{channel_type = binary_to_list(Type),
                          sender_channel = Sender,
                          initial_window_size = Window,
                          maximum_packet_size = Max,
                          data = Data};
decode(<<?BYTE(?SSH_MSG_CHANNEL_OPEN_CONFIRMATION),
         ?UINT32(Recipient),
         ?UINT32(Sender),
         ?UINT32(InitWindowSize),
         ?UINT32(MaxPacketSize),
         Data/binary>>) ->
    #ssh_msg_channel_open_confirmation{recipient_channel = Recipient,
                                       sender_channel = Sender,
                                       initial_window_size = InitWindowSize,
                                       maximum_packet_size = MaxPacketSize,
                                       data = Data};
decode(<<?BYTE(?SSH_MSG_CHANNEL_OPEN_FAILURE),
         ?UINT32(Recipient),
         ?UINT32(Reason),
         ?DEC_BIN(Desc, __0),
         ?DEC_BIN(Lang, __1)>>) ->
    #ssh_msg_channel_open_failure{recipient_channel = Recipient,
                                  reason = Reason,
...
decode(<<?BYTE(?SSH_MSG_CHANNEL_EXTENDED_DATA),
         ?UINT32(Recipient),
         ?UINT32(DataType),
         ?DEC_BIN(Data, __0)>>) ->
    #ssh_msg_channel_extended_data{recipient_channel = Recipient,
                                   data_type_code = DataType,
                                   data = Data};
...
decode(<<?BYTE(?SSH_MSG_CHANNEL_REQUEST),
         ?UINT32(Recipient),
         ?DEC_BIN(RequestType, __0),
         ?BYTE(Bool),
         Data/binary>> =
           Bytes) ->
    try
        #ssh_msg_channel_request{recipient_channel = Recipient,
...
decode(<<?BYTE(?SSH_MSG_USERAUTH_REQUEST),
         ...
         ?DEC_BIN(Method, __2),
         Data/binary>>) ->
    #ssh_msg_userauth_request{user = ?unicode_list(User),
                              service = binary:bin_to_list(Service),
                              method = binary:bin_to_list(Method),
                              data = Data};
decode(<<?BYTE(?SSH_MSG_USERAUTH_FAILURE), ?DEC_BIN(Auths, __0), ?BYTE(Bool)>>) ->
    #ssh_msg_userauth_failure{authentications = binary:bin_to_list(Auths),
...
decode(<<?BYTE(?SSH_MSG_USERAUTH_PASSWD_CHANGEREQ),
         ?DEC_BIN(Prompt, __0),
         ?DEC_BIN(Lang, __1)>>) ->
    #ssh_msg_userauth_passwd_changereq{prompt = Prompt, language = Lang};
%%% Unhandled message, also masked by same 1:st byte value as ?SSH_MSG_USERAUTH_INFO_REQUEST:
...
decode(<<?BYTE(?SSH_MSG_EXT_INFO), ?UINT32(N), BinData/binary>>)
    ...
    Data =
        bin_foldr(fun (Bin, Acc) when length(Acc) == N ->
                          {Bin, Acc};
...
decode(<<"dh",
         ...
         ?DEC_BIN(Key, KLen),
         ?DEC_MPINT(F, FLen),
         ?DEC_BIN(Hashsign, HLen)>>)
    when KLen =< ?MAX_HOST_KEY_SIZE, FLen =< ?MAX_DH_MPINT_SIZE,
         HLen =< ?MAX_SIGNATURE_SIZE ->
    #ssh_msg_kexdh_reply{public_host_key = ssh2_pubkey_decode(Key),
                         f = F,
...
decode(<<"dh",
         ...
         ?DEC_BIN(_, HLen)>>) ->
    throw({error,
           size_error([{kexdh_host_key_too_large, KLen, ?MAX_HOST_KEY_SIZE},
...
decode(<<"dh_gex",
         ...
         ?DEC_MPINT(Prime, PLen),
         ?DEC_MPINT(Generator, GLen)>>)
...
decode(<<"dh_gex",
         ...
         ?DEC_MPINT(_, GLen)>>) ->
    throw({error,
           size_error([{gex_p_too_large, PLen, ?MAX_DH_MPINT_SIZE},
...
decode(<<?BYTE(?SSH_MSG_KEX_DH_GEX_REPLY),
         ?DEC_BIN(Key, KLen),
         ?DEC_MPINT(F, FLen),
         ?DEC_BIN(Hashsign, SLen)>>)
    when KLen =< ?MAX_HOST_KEY_SIZE, FLen =< ?MAX_DH_MPINT_SIZE,
         SLen =< ?MAX_SIGNATURE_SIZE ->
    #ssh_msg_kex_dh_gex_reply{public_host_key = ssh2_pubkey_decode(Key),
                              f = F,
                              h_sig = decode_signature(Hashsign)};
decode(<<"dh_gex",
         ...
         ?DEC_BIN(_, SLen)>>) ->
    throw({error,
           size_error([{gex_host_key_too_large, KLen, ?MAX_HOST_KEY_SIZE},
...
decode(<<"ecdh",
         ...
         ?DEC_BIN(Q_s, QLen),
         ?DEC_BIN(Sig, SLen)>>)
    when KLen =< ?MAX_HOST_KEY_SIZE, QLen =< ?MAX_ECDH_POINT_SIZE,
         SLen =< ?MAX_SIGNATURE_SIZE ->
    #ssh_msg_kex_ecdh_reply{public_host_key = ssh2_pubkey_decode(Key),
                            q_s = Q_s,
...
decode(<<"ecdh",
         ...
         ?DEC_BIN(_, SLen)>>) ->
    throw({error,
           size_error([{ecdh_host_key_too_large, KLen, ?MAX_HOST_KEY_SIZE},
...
decode(<<"mlkem",
         ...
         ?DEC_BIN(Key, KLen),
         ?DEC_BIN(S_reply, SLen),
         ?DEC_BIN(Sig, SigLen)>>)
    when KLen =< ?MAX_HOST_KEY_SIZE, SLen =:= ?MLKEM768_REPLY_SIZE,
...
decode(<<"mlkem",
         ...
         ?DEC_BIN(_, SigLen)>>) ->
    throw({error,
           size_error([{mlkem_host_key_too_large, KLen, ?MAX_HOST_KEY_SIZE},
...
decode(<<?BYTE(?SSH_MSG_DISCONNECT),
         ?UINT32(Code),
         ?DEC_BIN(Desc, DLen),
         ?DEC_BIN(Lang, LLen)>>)
...
decode(<<?BYTE(?SSH_MSG_DISCONNECT),
         ...
         ?DEC_BIN(_, LLen)>>) ->
    throw({error,
           size_error([{disconnect_desc_too_large, DLen, ?MAX_DISCONNECT_DESC_SIZE},
...
decode(<<?BYTE(?SSH_MSG_DEBUG), ?BYTE(_), ?DEC_BIN(_, MLen), ?DEC_BIN(_, LLen)>>) ->
    throw({error,
           size_error([{debug_msg_too_large, MLen, ?MAX_DEBUG_MSG_SIZE},
...
ssh2_pubkey_encode(#'ECPrivateKey'{parameters = {namedCurve, OID}, publicKey = Key})
    when OID == ?'id-Ed25519' orelse OID == ?'id-Ed448' ->
    {KeyType, _} = oid2ssh_curvename(OID),
...
ssh2_pubkey_decode2(<<?DEC_BIN(SshCurveName, SCNL), Rest0/binary>>) ->
    {Pub, Rest} =
        case {SshCurveName, Rest0} of
            {<<"ecdsa-sha2-", _/binary>>,
             <<?DEC_BIN(_Curve, _IL), ?DEC_BIN(Q, _QL), Rest1/binary>>} ->
                {Q, Rest1};
            {<<"ssh-ed", _/binary>>, <<?DEC_BIN(Key, _L), Rest1/binary>>} ->
                {Key, Rest1}
        end,
...
ssh2_privkey_encode(#'RSAPrivateKey'{version =
                                         'two-prime', % Found this in public_key:generate_key/1 ..
                                     modulus = N,
                                     publicExponent = E,
                                     privateExponent = D,
                                     prime1 = P,
    ...
    <<?STRING(<<"ssh-rsa">>),
      ...
      ?Empint(P),
      ?Empint(Q)>>;
ssh2_privkey_encode(#'DSAPrivateKey'{version = 0,
                                     p = P,
                                     q = Q,
                                     g = G,
                                     y = Y,
...
ssh2_privkey_encode(#'ECPrivateKey'{version = Version,
    ...
    <<?STRING(CurveName), ?STRING(Pub), ?STRING(Priv)>>;
ssh2_privkey_encode(#'ECPrivateKey'{version = Version,
                                    parameters = {namedCurve, OID},
                                    privateKey = Priv,
                                    publicKey = Q})
    when Version =:= ecPrivkeyVer1
         orelse Version =:= 1 -> % currently used atom and legacy value 1
    {CurveName, _} = oid2ssh_curvename(OID),
    <<?STRING(CurveName),
            ...
            {<<"ecdsa-sha2-", _/binary>>,
             <<?DEC_BIN(_Curve, _IL),
               ...
               Rest1/binary>>} ->
                {Pub1, Priv1, Rest1};
            {<<"ssh-ed", _/binary>>,
             <<?DEC_BIN(Pub1, PL), ?DEC_BIN(PrivPub, PPL), Rest1/binary>>} ->
                PL = PPL div 2,
...
decode_kex_init(<<?DEC_BIN(Data, __0), Rest/binary>>, Acc, N)
    when byte_size(Data) < ?MAX_NUM_ALGORITHMS * ?ALG_NAME_LIMIT ->
...
ssh_dbg_on(P) when P == ssh_messages; P == raw_messages ->
    dbg:tp(?MODULE, encode, 1, x),
    dbg:tp(?MODULE, decode, 1, x).

ssh_dbg_off(P) when P == ssh_messages; P == raw_messages ->
...
ssh_dbg_format(ssh_messages, {return_from, {?MODULE, decode, 1}, Msg}) ->
    ...
    ["Received ",
     ...
     case Msg of
         ...
         #ssh_msg_channel_request{request_type = "exec", data = <<?DEC_BIN(Cmnd, __0)>>} ->
