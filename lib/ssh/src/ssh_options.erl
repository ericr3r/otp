...
-export([default/1, get_value/5, get_value/6, put_value/5, delete_key/5, handle_options/2,
...
%%%
merge_options(Role, NewPropList, Opts0) when is_list(NewPropList), is_map(Opts0) ->
...
check_and_save(OptsList, OptionDefinitions, InitialMap) ->
    final_preferred_algorithms(lists:foldl(fun(KV, Vals) -> save(KV, OptionDefinitions, Vals)
                                           end,
                                           InitialMap,
                                           OptsList)).
...
no_sensitive(rm, #{id_string := _, tstflg := _}) ->
    '*** removed ***';
no_sensitive(filter, Opts = #{id_string := _, tstflg := _}) ->
    Sensitive =
        [password,
         user_passwords,
         dsa_pass_phrase,
...
check_silently_accept_hosts(B) when is_boolean(B) ->
    true;
check_silently_accept_hosts(F) when is_function(F, 2) ->
    true;
check_silently_accept_hosts({false, S}) when is_atom(S) ->
    valid_hash(S);
check_silently_accept_hosts({S, F}) when is_function(F, 2) ->
    valid_hash(S);
...
nml(K, L) -> 
    ...
    case L -- lists:usort(L) of
        [] ->
            ok;
        Dups ->
            error_in_check({K, Dups}, "Duplicates") % This is a throw
    end,
    L.
...
final_preferred_algorithms(Options0) ->
    Result =
        case ?GET_OPT(modify_algorithms, Options0) of
            undefined ->
                rm_non_supported(true, ?GET_OPT(preferred_algorithms, Options0));
            ModAlgs ->
...
check_subsystem({"sftp", {ssh_sftpd, Args}}) ->
    ...
    case Root =:= "" orelse filename:pathtype(Root) =:= absolute of
        true ->
            true;
        false ->
            error_in_check({"sftp", {ssh_sftpd, Args}},
                           io_lib:format("SFTP root option must be an absolute path, got: ~p",
                                         [Root]))
    end;
check_subsystem(_) ->
    true.
